...
close all
clearvars -except resf project_dir results_dir datadir hpfs lpfs f_ind frq_cols pval all_real_diffs all_mean_within_corrs band_name min_corr max_corr
...
for n = 1:10
    for m = 1:10
        ...
        % dilution correction
        xx1_mat = AEC_b_all(:,:,n);
        xx2_mat = AEC_b_all2(:,:,n);
        yy1_mat = AEC_b_all(:,:,m);
        yy2_mat = AEC_b_all2(:,:,m);
        
        xx_corr = corr(xx1_mat(mask),xx2_mat(mask))
        yy_corr = corr(yy1_mat(mask),yy2_mat(mask))
        sqrt((xx_corr*yy_corr))
%         if ~isreal(sqrt((xx_corr*yy_corr)))
%             pause
%         end
        if n == m
        sub_correl(n,m) = corr(run1sa_v_t,run2sb_v_t);
        else
        sub_correl(n,m) = corr(run1sa_v_t,run2sb_v_t)./sqrt(abs(xx_corr*yy_corr)) 
        end
    end
end
min_corr = -0.15 %min(sub_correl(:))
max_corr = 1 %max(sub_correl(:))
min_corr = min(sub_correl(:))
max_corr = max(sub_correl(:))
