...
function Copy_of_data_cleaning_test(sub,ses)
restoredefaultpath
cleaning_only = 0;
close all
clc
...
if ~cleaning_only
    ...
    for f_i = 1:length(hpfs)
        if ~exist(sprintf('%s%s_%d_%d_Hz.mat',path.AEC,files.AEC,hp,lp),'file')
        % hp = 13;
        % lp = 30;
        hp = hpfs(f_i);
        lp = lpfs(f_i);
        ...
        save(sprintf('%s%s_%d_%d_Hz.mat',path.VEs,files.VEs,hp,lp),'VE')
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%
        down_f = 10;
        Nlocs = 78;
        ...
        save(sprintf('%s%s_%d_%d_Hz.mat',path.AEC,files.AEC,hp,lp),'AEC')
        end
    end
    
    hp = 1;
    lp = 150;
    if ~exist(sprintf('%s%s_%d_%d_Hz.mat',path.VEs,files.VEs,hp,lp),'file')
    data_f = data_f_clean';
    %% Beamform
    C = cov(data_f);
    mu = 0.05;
