function [bf_outs] = run_beamformer_CTF(fwd_method,sourcepos,S,do_bf,max_or_min,MFC)
...
% S.rad_inds      = Radial channel (Z) indexes (e.g. if all radial then
%                   1:Nchans, if all dual 2:2:Nchans).
% S.tan_inds      = Tangential channel (Y) indexes. If there isn't a
%                   tangential channel, make it equal to the radial channel.
%
% For BEM
...
elseif strcmpi(fwd_method,'local')
    ...
    if exist([path.skanect,'local_spheres_info_',filename.skanect(end-5:end),'.mat'])
        ...
        sensors.pos = S.sensor_info.pos(S.rad_inds,:);
        sensors.ors = S.sensor_info.ors(S.rad_inds,:);
        [so1, sr1] = create_local_spheres(brain_mesh,sensors);
        % duplicate the sphere origins for Dual mode
        if isfield(S,'tan_inds')
            for n = 1:length(S.rad_inds)
                Loc_ind = unique([S.tan_inds(n) S.rad_inds(n)]);
...
if do_bf   
    clear LF ltan
    count = 0;
    LF_grad = zeros(size(bf_outs.LF)./[2 1 1]);
    for n = 1:2:size(bf_outs.LF,1)
        count = count+1;
        LF_grad(count,:,:) = bf_outs.LF(n+1,:,:) + bf_outs.LF(n,:,:); % Sum because they're wired opposite
    end
