function [lead_fields,UnitMDip] = Forward_OPM_local_spheres_XYZ_dir(x,y,z,slots_all,normal_unit,sphere_origins,tan_ref,tan_method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% number of channels
...
if strcmpi(tan_method,'norm')
    angles = (1:(180/N):180)./180.*pi;
    [etheta,ephi] = calctangent(tan_ref);
    u=cos(angles);
    v=sin(angles);    
    for ix=1:3
        MDip(:,ix)=u.*etheta(ix)+v.*ephi(ix);
