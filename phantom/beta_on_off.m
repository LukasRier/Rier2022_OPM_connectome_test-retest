...
imwrite(im,map,'Animation_rand_sim_beta.gif','DelayTime',0.1,'LoopCount',inf)
...
lp = 100;
[b,a] = butter(4,2*[hp lp]/f);
OPM_data_f_100 = [filtfilt(b,a,OPM_data_f')]';
...
Cc = Ca;
for tr_i = 1:size(beginning_samples,2)
    ...
    Cc(:,:,tr_i) = cov(data_f_mat(:,trl_duration_samps+1:trl_duration_samps*2 -1,tr_i)');
    active_pow(:,:,tr_i) = abs(hilbert(data_f_mat(:,1:trl_duration_samps-1,tr_i)'));
    control_pow(:,:,tr_i) = abs(hilbert(data_f_mat(:,trl_duration_samps+1:trl_duration_samps*2 -1,tr_i)'));

end
Ca = mean(Ca,3);
Cc = mean(Cc,3);
sensor_pow = (active_pow - control_pow)./control_pow;
mean_sensor_pow = mean(mean(sensor_pow,3),1);
C =mean((Ca + Cc)./2,3);
mu = 0.01;
Cr = C + mu*max(svd(C))*eye(size(C));
Cr_inv = inv(Cr);

pseudoT = zeros(Ndips,1)
for n = 1:Ndips
    ...
    pseudoT(n) =  ((w*Ca*w') - (w*Cc*w'))./(2*(w*Cc*w'));
end
% find max and calculate VE
n = find(pseudoT==max(pseudoT));
